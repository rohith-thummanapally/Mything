    ...
    async getcategorywiseExpenses(userid, fromdate, todate, categories, tags) {
        try {
            ...
            if (tags && tags.length > 0 && 1 == 0) {
                tagcond = ' and find_in_set( ?,be.tags) ';
                variables.push(tags.join(','));
            }
            let fromdatecond = '';
            if (fromdate) {
            ...
            let tagwiseexpenses = await db.query(`select bc.id,bc.category_name,sum(be.amount) as amount  from base_expenses be join base_categories bc on bc.id=be.category where be.userid=? ${categorycond} ${tagcond} ${fromdatecond} ${todatecond} group by bc.id `, variables);
            return tagwiseexpenses[0];
        }
        catch (err) {
            console.log(err);
